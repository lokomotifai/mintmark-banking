...
- **Lexicons that reached no generator.** `banks_fictional`, `products_fictional`, `occupations_bank_tr` and `complaint_topics_tr` were shipped,
...
- Birth dates are drawn from an age window rather than from the recipe window.
  Every customer in a dataset describing 2026 was also born in 2026, and no test
  could have caught it: the field is a valid date, the label is right, the span
  aligns, the manifest verifies. It is only wrong to a reader.
- A real Turkish insurer that had reached the fictional lexicon was removed, and
  the root of it was corrected in the counterparty list as well.
